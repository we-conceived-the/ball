...
*Setup instructions for a Gitian build of Ball Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Ball
...
- [Building Ball Core](#building-ball-core)
...
Clone the git repositories for Ball Core and Gitian.
...
in the Ball Core repository under 'Fetch and create inputs' to install sources which require
...
To build Ball Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Ball Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/ball/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://we-conceived-the/ball
...
For example:
```bash
URL=https://github.com/crowning-/ball.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit ball=${COMMIT} --url ball=${URL} ../ball/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit ball=${COMMIT} --url ball=${URL} ../ball/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit ball=${COMMIT} --url ball=${URL} ../ball/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the ball git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../ball/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/ball
SIGPATH=/some/root/path/ball-detached-sigs

./bin/gbuild --url ball=${BTCPATH},signature=${SIGPATH} ../ball/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/ball-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ball-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/ball-osx-build.assert
```
...
[ball/gitian.sigs](https://github.com/ballpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
