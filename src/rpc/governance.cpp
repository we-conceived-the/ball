...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in ball.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("ball.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Ball features */
    { "ball",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "ball",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "ball",               "gobject",                &gobject,                true,  {} },
    { "ball",               "voteraw",                &voteraw,                true,  {} },
