...
const char * const BITCOIN_CONF_FILENAME = "ball.conf";
const char * const BITCOIN_PID_FILENAME = "balld.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "ball" is a composite category enabling all Ball-related debug output
                if(ptrCategory->count(std::string("ball"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BallCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BallCore
    // Mac: ~/Library/Application Support/BallCore
    // Unix: ~/.ballcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BallCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BallCore";
#else
    // Unix
    return pathRet / ".ballcore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty ball.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override ball.conf
